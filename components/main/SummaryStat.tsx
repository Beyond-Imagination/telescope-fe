...
function DivSummary({ number, text, color }: any) {
    return (
        <div className="flex flex-col w-[120px] items-center gap-1">
            <span className="font-bold text-[36px] text-[#FFF]">{number ? number : 0}</span>
            <span className="font-bold text-[12px] text-[#9A9EA6]" style={{ color: color }}>
                {text}
            </span>
        </div>
    )
...
function SummaryStat({ total, create_issue, resolve_issue, create_code_review, merge_mr, receive_star, timeType, setTimeType }: any) {
    return (
        <div className={`summaryStat`}>
            <div className={`mb-3 flex justify-between`}>
                <div className={`flex items-center pl-3 scoreInfo`}>
                    <span className={`font-normal text-[17px] mr-3.5`} style={{ fontWeight: 700, color: '#23222C' }}>
                        Score
                    </span>
            ...
            <div className={`w-full h-[96px] flex justify-center shadow-[0_5px_20px_1px_rgba(0,0,0,0.1)] rounded mb-[12px] bg-[#23222C] scoreFrame`}>
